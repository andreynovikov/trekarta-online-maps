...
public class ChartBundleOnlineMapProvider extends ContentProvider {
    private final static String MAPS_URL = "http://wms.chartbundle.com/tms/1.0.0/%s/%d/%d/%d.png?type=google";
    private final String[] maps = {"sec", "tac", "wac", "enrl", "enrh", "enra"};
            ...
            for (String column : projection) {
                switch (column) {
                    ...
                    case "NAME":
                        ...
                        break;
                    case "MIN_ZOOM":
                        id = resources.getIdentifier(map + "_minzoom", "integer", packageName);
                        int minZoom = resources.getInteger(id);
                        row.add(minZoom);
                        break;
                    case "MAX_ZOOM":
                        id = resources.getIdentifier(map + "_maxzoom", "integer", packageName);
                        int maxZoom = resources.getInteger(id);
                        row.add(maxZoom);
                        break;
                }
            }
